...
const PLECA_EASE = [0.25, 0.1, 0.25, 1];
const PLECA_DURATION = 0.82;
const CONTENT_REVEAL_DELAY = PLECA_DURATION * 2;

const BADGES = [
  { key: "cuenta", src: "/images/comunidad/circulo-cuenta.png", alt: "Cada alumno cuenta", label: "CADA ALUMNO CUENTA", left: 763, top: 148, delay: 0.62, float: 4 },
  { key: "comunidad", src: "/images/comunidad/circulo-comunidad.png", alt: "Crecemos en comunidad", label: "CRECEMOS EN COMUNIDAD", left: 569, top: 326, delay: 0.72, float: 5 },
  { key: "mente", src: "/images/comunidad/circulo-mente.png", alt: "Desarrollamos mente, cuerpo y carácter", label: "DESARROLLAMOS MENTE, CUERPO Y CARÁCTER", left: 820, top: 471, delay: 0.82, float: 4.5 },
  { key: "haciendo", src: "/images/comunidad/circulo-haciendo.png", alt: "Aprendemos haciendo", label: "APRENDEMOS HACIENDO", left: 1090, top: 438, delay: 0.92, float: 5.5 },
];
...
function AnimatedBadge({ badge, play, reduceMotion, children, className, style }) {
  return (
    <motion.div
      ...
      initial={false}
      animate={{ opacity: play ? 1 : 0, scale: play || reduceMotion ? 1 : 0.82 }}
      transition={reduceMotion
        ? { duration: 0.01 }
        : { duration: 0.48, delay: badge.delay, ease: EASE_OUT }}
    >
      <motion.div
        className="relative h-full w-full"
        animate={play && !reduceMotion ? { y: [0, -badge.float, 0, badge.float * 0.45, 0] } : { y: 0 }}
        transition={play && !reduceMotion
          ? { duration: 5.4 + badge.float * 0.12, delay: badge.delay + 0.5, repeat: Infinity, ease: "easeInOut" }
          : { duration: 0 }}
      >
        {children}
      </motion.div>
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        initial={false}
        animate={{ x: play || reduceMotion ? "0%" : "-100%" }}
        transition={{ duration: reduceMotion ? 0.01 : PLECA_DURATION, ease: PLECA_EASE }}
      >
          <div className="absolute" style={{ left: x(-4), top: y(-1), width: unit(651.458), height: unit(772) }}>
            <Image src="/images/comunidad/pleca-izquierda-figma-790-6490.svg" alt="" fill loading="eager" sizes="46vw" className="object-fill" />
          </div>
          <div className="absolute" style={{ left: x(390.116), top: y(2.012), width: unit(264.117), height: unit(759.988) }}>
            <Image src="/images/comunidad/borde-blanco-izquierdo-figma-790-6490.svg" alt="" fill loading="eager" sizes="19vw" className="object-fill" />
          </div>
      </motion.div>
      <motion.div
        className="absolute inset-0 transform-gpu"
        initial={false}
        animate={{ x: play || reduceMotion ? "0%" : "100%" }}
        transition={{ duration: reduceMotion ? 0.01 : PLECA_DURATION, delay: reduceMotion ? 0 : PLECA_DURATION, ease: PLECA_EASE }}
      >
          <div className="absolute flex items-center justify-center" style={{ left: x(661), top: y(-7), width: unit(778), height: unit(409.174) }}>
            <div className="relative flex-none" style={{ width: unit(419.174), height: unit(789), transform: "rotate(90deg)" }}>
              <Image src="/images/comunidad/pleca-derecha-figma-790-6490.svg" alt="" fill loading="eager" sizes="55vw" className="object-fill" />
            </div>
          </div>
          <div className="absolute flex items-center justify-center" style={{ left: x(661), top: y(2), width: unit(778), height: unit(409.174) }}>
            <div className="relative flex-none" style={{ width: unit(409.162), height: unit(777.989), transform: "rotate(90deg)" }}>
              <Image src="/images/comunidad/borde-blanco-derecho-figma-790-6490.svg" alt="" fill loading="eager" sizes="55vw" className="object-fill" />
            </div>
          </div>
      </motion.div>
      ...
      <motion.div className="absolute" style={{ left: x(226), top: y(326), width: unit(359), height: unit(434) }} initial={false} animate={{ opacity: play ? 1 : 0 }} transition={{ duration: reduceMotion ? 0.01 : 0.5, delay: reduceMotion ? 0 : 0.48, ease: EASE_OUT }}>
      ...
      <motion.div
        ...
        style={{ left: x(101), top: y(68), gap: unit(5), willChange: "clip-path" }}
        initial={false}
        animate={{ clipPath: play ? "inset(0% 0% 0% 0%)" : "inset(0% 0% 100% 0%)" }}
        transition={{ duration: reduceMotion ? 0.01 : 0.62, delay: reduceMotion ? 0 : CONTENT_REVEAL_DELAY, ease: EASE_OUT }}
      >
      ...
      <motion.div
        ...
        style={{ left: x(101), top: y(107), width: unit(407), willChange: "clip-path" }}
        initial={false}
        animate={{ clipPath: play ? "inset(0% 0% 0% 0%)" : "inset(0% 0% 100% 0%)" }}
        transition={{ duration: reduceMotion ? 0.01 : 0.72, delay: reduceMotion ? 0 : CONTENT_REVEAL_DELAY, ease: EASE_OUT }}
      >
...
function ResponsiveArtwork({ play, reduceMotion }) {
  return (
    <motion.div className="relative isolate overflow-hidden lg:hidden" style={{ background: SKY }} initial={false} animate={{ opacity: play ? 1 : 0 }} transition={{ duration: 0 }}>
      ...
      >
          <div className="absolute -left-[28%] -top-3 h-[58%] w-[112%] sm:-left-[18%] sm:w-[84%]">
            <Image src="/images/comunidad/pleca-izquierda-figma-790-6490.svg" alt="" fill loading="eager" sizes="90vw" className="object-fill" />
          </div>
          <div className="absolute -left-[3%] top-0 h-[62%] w-[58%] opacity-90 sm:w-[46%]">
            <Image src="/images/comunidad/borde-blanco-izquierdo-figma-790-6490.svg" alt="" fill loading="eager" sizes="55vw" className="object-fill" />
          </div>
      </motion.div>
      <motion.div
        className="absolute inset-0 -z-10 transform-gpu"
      ...
      >
          <div className="absolute -right-[40%] top-[32%] h-[38%] w-[130%] rotate-90 sm:-right-[28%] sm:w-[90%]">
            <Image src="/images/comunidad/pleca-derecha-figma-790-6490.svg" alt="" fill loading="eager" sizes="100vw" className="object-fill" />
          </div>
          <div className="absolute -right-[23%] top-[39%] h-[34%] w-[72%] rotate-90 opacity-90">
            <Image src="/images/comunidad/borde-blanco-derecho-figma-790-6490.svg" alt="" fill loading="eager" sizes="70vw" className="object-fill" />
          </div>
      </motion.div>
      ...
      <div className="mx-auto flex w-full max-w-3xl flex-col px-5 pb-16 pt-12 sm:px-10 sm:pb-20 sm:pt-16">
        ...
        <motion.div className="relative -mt-1 flex justify-center sm:-mt-5 sm:justify-start sm:pl-12" initial={false} animate={{ opacity: play ? 1 : 0 }} transition={{ duration: reduceMotion ? 0.01 : 0.5, delay: reduceMotion ? 0 : 0.48, ease: EASE_OUT }}>
...
export default function Comunidad() {
  ...
  return (
    <section ref={sectionRef} className="relative -mt-px bg-[#fafaf7]">
      <DesktopArtwork play={play && isDesktop === true} reduceMotion={reduceMotion} />
      <ResponsiveArtwork play={play && isDesktop === false} reduceMotion={reduceMotion} />
    </section>
  );
}
