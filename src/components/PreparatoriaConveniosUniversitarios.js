...
function Balloon({ university, mobile = false }) {
  const position = mobile ? undefined : { left: `${university.x / 14.4}%`, top: `${university.y / 7.6}%`, width: `${147 / 14.4}%` };
  return <div className={mobile ? "relative aspect-[147/198] w-full" : "absolute aspect-[147/198]"} style={position}>
  ...
  return <section aria-labelledby="conexion-universitaria" className="relative overflow-hidden bg-[linear-gradient(89.7deg,#637e99_3.8%,#8dbad7_30.9%)]">
    <div className="relative hidden aspect-[1440/760] w-full lg:block">
      ...
      <div className="absolute left-[7.15%] top-[7.75%] z-10 w-[30.7%]">
        <div className="mb-[5px] flex items-center gap-[14px]"><span className="h-[6px] w-14 bg-red-700" /><p className="whitespace-nowrap text-[clamp(11px,1.05vw,15px)] text-white [text-shadow:0_4px_4px_rgba(0,0,0,.25)]">CONEXIÓN UNIVERSITARIA</p></div>
        <h2 id="conexion-universitaria" className="font-display text-[clamp(34px,3.2vw,46px)] font-medium leading-[.96] text-white [text-shadow:0_4px_4px_rgba(0,0,0,.25)]">Construimos puentes para ampliar sus posibilidades</h2>
      </div>
      {universities.map((university) => <Balloon key={university.alt} university={university} />)}
    </div>
    <div className="relative px-5 pb-20 pt-12 lg:hidden">
      ...
      <div className="relative z-10 mb-8"><div className="mb-2 flex items-center gap-3"><span className="h-1 w-10 bg-red-700" /><p className="text-xs text-white">CONEXIÓN UNIVERSITARIA</p></div><h2 className="max-w-md font-display text-4xl font-medium leading-none text-white [text-shadow:0_3px_4px_rgba(0,0,0,.25)]">Construimos puentes para ampliar sus posibilidades</h2></div>
      <div className="relative z-10 grid grid-cols-3 gap-x-2 gap-y-1 sm:grid-cols-4">{universities.map((university) => <Balloon key={university.alt} university={university} mobile />)}</div>
    </div>
  </section>;
}
